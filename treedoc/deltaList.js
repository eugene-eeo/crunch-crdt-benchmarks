...
var addTombstone = function (state, d, delID) {
    var replicaDeletes = state.removes[delID.r] || {};
    if (!replicaDeletes) {
        state.removes[delID.r] = replicaDeletes;
    }
    replicaDeletes[delID.o] = d;

    var newMax = delID.o;
    if (state.removed[delID.r]) {
        newMax = Math.max(newMax, state.removed[delID.r]);
    }
    state.removed[delID.r] = newMax;
};
...
const DS_TableList = require('./DS_TableList.js');
...
module.exports = {
    ...
    crdt: {
        ...
        getDelta: function (vv, meta) {
            ...
            var newRemoves = [];
            var myRemoves = Object.keys(this.state.removed);

            for (var j = 0; hisRemoves && j < hisRemoves.length; j++) {
                var k = hisRemoves[j][0];
                var v = hisRemoves[j][1];
                if (this.state.removed[k] > v) {
                    for (var ri = v + 1; ri <= this.state[k]; ri++) {
                        newRemoves.push([{r: k, o: ri}, this.state.removes[k][ri]]);
                    }
                }
            }
            ...
            for (var j = 0; j < myRemoves.length; j++) {
                ...
                if (!found) {
                    for (var ri = 1; ri <= this.state.removed[myRemoves[j]]; ri++) {
                        newRemoves.push([{r: myRemoves[j], o: ri}, this.state.removes[myRemoves[j]][ri]]);
                    }
                }
            }
        ...
        applyDelta: function (delta, vv, meta) {
            ...
            try {
                ...
                for (var j = 0; j < newRemoves.length; j++) {
                    ...
                    if (this.state.removes[itemID.r] && this.state.removes[itemID.r][itemID.o]) {
        ...
        getMeta: function () {
            return {r: Object.entries(this.state.removed)}
        }
    }
};
